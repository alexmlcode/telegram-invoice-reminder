...
import logging
import os, sys, json, time, uuid, pathlib, subprocess, datetime, threading, queue as _queue_mod, re
...
# Load secrets from .env file (fallback to env vars for local runs)
try:
    dotenv.load_dotenv()
except Exception:
    pass
...
# Fallback: try to import colab modules for backward compatibility
...
def _is_openrouter_base_url(url: Optional[str]) -> bool:
    return "openrouter.ai" in _normalized(url).lower()


def _parse_github_repo_from_remote(remote_url: str) -> Tuple[Optional[str], Optional[str]]:
    """
    Parse GitHub user/repo from common remote URL formats.
    Supports:
...
LLM_BASE_URL = get_cfg("OUROBOROS_BASE_URL", default="https://openrouter.ai/api/v1", allow_legacy_secret=True)
OPENROUTER_API_KEY = _normalized(get_secret("OPENROUTER_API_KEY", default=""))
TELEGRAM_BOT_TOKEN = get_secret("TELEGRAM_BOT_TOKEN", required=True)
TOTAL_BUDGET_DEFAULT = get_secret("TOTAL_BUDGET", required=True)
GITHUB_TOKEN = _normalized(get_secret("GITHUB_TOKEN", default=""))

# OpenRouter key is required only when routing through OpenRouter.
if _is_openrouter_base_url(LLM_BASE_URL) and not OPENROUTER_API_KEY:
    raise AssertionError("Missing required secret: OPENROUTER_API_KEY (required for OpenRouter base URL)")
...
OPENAI_API_KEY = _normalized(get_secret("OPENAI_API_KEY", default=""))
ANTHROPIC_API_KEY = _normalized(get_secret("ANTHROPIC_API_KEY", default=""))
GIT_REMOTE_URL = _normalized(get_cfg("OUROBOROS_GIT_REMOTE_URL", default="", allow_legacy_secret=True))
_remote_user, _remote_repo = _parse_github_repo_from_remote(GIT_REMOTE_URL)
GITHUB_USER = get_cfg("GITHUB_USER", default=(_remote_user or "razzant"), allow_legacy_secret=True)
GITHUB_REPO = get_cfg("GITHUB_REPO", default=(_remote_repo or "ouroboros"), allow_legacy_secret=True)
BRANCH_DEV = str(get_cfg("OUROBOROS_BRANCH_DEV", default="ouroboros", allow_legacy_secret=True) or "ouroboros")
BRANCH_STABLE = str(
    get_cfg("OUROBOROS_BRANCH_STABLE", default="ouroboros-stable", allow_legacy_secret=True) or "ouroboros-stable"
)
...
if MODEL_LIGHT is None:
    MODEL_LIGHT = DEFAULT_LIGHT_MODEL
...
os.environ["OUROBOROS_BASE_URL"] = str(LLM_BASE_URL or "https://openrouter.ai/api/v1")
os.environ["OUROBOROS_BRANCH_DEV"] = BRANCH_DEV
os.environ["OUROBOROS_BRANCH_STABLE"] = BRANCH_STABLE
...
# ----------------------------
DRIVE_ROOT = pathlib.Path(os.environ.get("DRIVE_ROOT", str(pathlib.Path.home() / ".ouroboros"))).resolve()
REPO_DIR = pathlib.Path(os.environ.get("OUROBOROS_REPO_DIR", str(pathlib.Path.home() / "ouroboros_repo"))).resolve()
...
import supervisor.linkedin_listener as _linkedin_listener
...
if restored_pending > 0:
    ...
    if st_boot.get("owner_chat_id"):
        send_with_budget(
            int(st_boot["owner_chat_id"]), f"♻️ Restored pending queue from snapshot: {restored_pending} tasks."
        )
...
# ----------------------------
# 6.4) Listener drain threads (tg-drain, email-drain, linkedin-drain)
# ----------------------------
# Each listener puts events in its queue; drain threads convert them to
# user_chat tasks immediately, independent of the bot-API long-poll cycle.
_drains.init(
